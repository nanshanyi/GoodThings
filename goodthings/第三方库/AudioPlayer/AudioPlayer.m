...
//  Share
//
//  Create by Lin Zhang on 11-4-26.
...
- (void)updateProgress
{
    if (streamer.progress <= streamer.duration ) {
        if (!_number) {
            self.number = [NSNumber numberWithFloat:0.0];
            self.totalTimes = [NSNumber numberWithInteger:0];
            [self addObserver:[MusicPlayerViewController shareInstance] forKeyPath:@"number" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:[MusicPlayerViewController shareInstance] forKeyPath:@"totalTimes" options:NSKeyValueObservingOptionNew context:nil];
        }
        self.totalTimes = @(streamer.duration);
        self.number = @(streamer.progress);
       
       // NSLog(@"%f",streamer.duration);
